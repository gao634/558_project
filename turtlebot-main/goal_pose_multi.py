...
## THIS CODE WOULD WORK ASSUMING THE LAUNCH FILE AND NAMESPACE IS CORRECT ##
...
def navigation(turtlebot_name, arucoID, goal_list): 
    ...
    while current_position_idx < len(goal_list):
        ...
        #instead of conversion with translate and scale, we use matrix function
        #x = (x + x_translate) * x_scale
        #y = (y - y_translate) * -y_scale
        ...
        # Translation
        # init_pose.pose.position.x = (init_pose.pose.position.x - x_translate) * x_scale
        # init_pose.pose.position.y = (init_pose.pose.position.y + y_translate) * y_scale
